...
describe Account do
  ...
  it "should return the current balance based on the last account_transaction" do
    account = Factory(:account)

    account.current_balance.should == 0

    Factory(:account_transaction, :amount => 100, :debit => false, :account => account)
    Factory(:account_transaction, :amount => 40, :debit => true, :account => account)
  ...
  describe "#calculate_balance" do
    before do
      ...
      Factory(:account_transaction, :amount => 100, :debit => false, :account => @account)
      account_transaction = Factory(:account_transaction, :amount => 40, :debit => true, :account => @account)
      account_transaction.update_attribute(:balance, 0)
    end

    it "calculates based on all account_transactions" do
      @account.current_balance.should == 0
  ...
  describe "#recalculate_balance_history!" do
    before do
      ...
      Factory(:account_transaction, :amount => 100, :debit => false, :account => @account).update_attribute(:balance, 0)
      Factory(:account_transaction, :amount => 40, :debit => true, :account => @account).update_attribute(:balance, 0)

    end

    it "recalculates and applies correct balances for all account_transactions" do
      @account.current_balance.should == 0
      @account.recalculate_balance_history!
      @account.current_balance.should == 60

      @account.account_transactions.first.balance.should == 100
      @account.account_transactions.last.balance.should == 60

    end

    it "recalculates and applies correct balances for all account_transactions when not 0" do
      @account.account_transactions.first.update_attribute(:balance, -50)
      @account.account_transactions.last.update_attribute(:balance, 25)
