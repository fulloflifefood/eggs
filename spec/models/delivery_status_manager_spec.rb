...
describe DeliveryStatusManager do
  ...
  before(:each) do
    ...
    Factory(:delivery, :status => 'notyetopen', :opening_at => Time.current - 10.minutes, :status_override => false)
    Factory(:delivery, :status => 'notyetopen', :opening_at => Time.current + 10.minutes, :status_override => false)
    Factory(:delivery, :status => 'notyetopen', :opening_at => Time.current - 10.minutes, :status_override => true)

    Factory(:delivery, :status => 'open', :closing_at => Time.current - 10.minutes, :status_override => false)
    Factory(:delivery, :status => 'open', :closing_at => Time.current + 10.minutes, :status_override => false)
    Factory(:delivery, :status => 'open', :closing_at => Time.current - 10.minutes, :status_override => true)

    Factory(:delivery, :status => 'finalized', :date => Time.current - 1.day, :status_override => false)
    Factory(:delivery, :status => 'finalized', :date => Time.current, :status_override => false)
    Factory(:delivery, :status => 'finalized', :date => Time.current + 1.day, :status_override => false)
    Factory(:delivery, :status => 'finalized', :date => Time.current - 1.day, :status_override => true)
