...
describe DeliveryExporter do
  ...
  it "should only export stock items with quantity" do
    @delivery.stock_items << Factory.create(:stock_item, :quantity_available => 0, :product_name => 'olive oil', :delivery => @delivery)

    @delivery.stock_items.size.should == 3
    @delivery.stock_items.with_quantity.size.should == 2

    csv = DeliveryExporter.get_csv(@delivery)
    csv.include?("eggs").should == true
    csv.include?("olive oil").should == false
  end
  ...
  describe "Separating out locations" do
    ...
    it "should not include columns for stock items with zero orders" do
      @delivery.stock_items << Factory.create(:stock_item, :quantity_available => 20, :product_name => 'olive oil', :delivery => @delivery)
      @order1.order_items << Factory(:order_item, :order => @order1, :stock_item => @delivery.stock_items.last, :quantity => 0)

      headers = DeliveryExporter.get_headers(@delivery)
      headers.detect{|header| header.include?('olive oil')}.should == nil

      totals = DeliveryExporter.get_totals(@delivery)
      totals.include?(0).should == false

      order_row = DeliveryExporter.get_order_row(@order1)
      order_row.size.should == 5 + @delivery.stock_items.with_orders.size + @delivery.delivery_questions.visible.size + 6
    end
